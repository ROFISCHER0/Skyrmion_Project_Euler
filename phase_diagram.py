...
def parse_l_subzones(spec):
    ...
    for zone_index, entry in enumerate(spec.split(";"), start=1):
        ...
        if zone_max < zone_min:
            raise ValueError(f"Lattice subzone entry {zone_index} has max < min: '{entry}'")
...
def build_point_tasks(H_vals, A_vals, default_L, max_dt, cfl_factor, l_subzones=None):
    """Generate independent sweep points for workers or job-array chunks."""
    tasks = []
    n_H = len(H_vals)
...
def empty_result_bundle(n_A, n_H, default_L):
    ...
    bundle = {
        ...
        "lattice_size_grid": np.full((n_A, n_H), int(default_L), dtype=np.int32),
        "records": [],
    }
...
def integrate_record(bundle, record):
    ...
    bundle["lattice_size_grid"][a_index, h_index] = record["lattice_size"]
...
def save_result_bundle(bundle, H_vals, A_vals, metadata, npz_path, csv_path, json_path):
    ...
    np.savez(
        ...
        lattice_size_grid=bundle["lattice_size_grid"],
        ...
        default_lattice_size=np.array(metadata["default_lattice_size"]),
        l_subzones_json=np.array(json.dumps(metadata["l_subzones"], sort_keys=True)),
...
def build_metadata(
    ...
    return {
        "run_name": run_name,
        "lattice_size": int(default_L),
        "default_lattice_size": int(default_L),
        "l_subzones": list(l_subzones or []),
        "n_H": int(len(H_vals)),
        "n_A": int(len(A_vals)),
...
def generate_phase_diagram(
    ...
    all_tasks = build_point_tasks(H_vals, A_vals, L, max_dt, cfl_factor, l_subzones=l_subzones)
    ...
    print(f"Default lattice size: L={L}  |  Workers: {workers}")
    if l_subzones:
    ...
    bundle = empty_result_bundle(n_A, n_H, L)
...
def load_records_csv(csv_path):
    ...
    with open(csv_path, newline="", encoding="utf-8") as handle:
        ...
        for row in reader:
            ...
            row["lattice_size"] = int(row["lattice_size"]) if row.get("lattice_size") else 0
...
def merge_phase_diagram_chunks(run_name, output_root=None, save_plot=True):
    ...
    H_vals = first["H_vals"]
    A_vals = first["A_vals"]
    default_L = int(first["default_lattice_size"]) if "default_lattice_size" in first else int(first["lattice_size"])
    workers = 0
    stored_l_subzones = []
    if "l_subzones_json" in first:
        stored_l_subzones = json.loads(str(first["l_subzones_json"]))

    bundle = empty_result_bundle(len(A_vals), len(H_vals), default_L)
    merged_records = []
    total_elapsed = 0.0
    ...
    for chunk_path in chunk_paths:
        ...
        if "lattice_size_grid" in chunk:
            bundle["lattice_size_grid"][mask] = chunk["lattice_size_grid"][mask]
        else:
            bundle["lattice_size_grid"][mask] = int(chunk["lattice_size"])
...
def plot_saved_phase_diagram(npz_path, out_path=None, show_plot=False, output_root=None):
    """Render a previously saved phase-diagram NPZ without recomputing anything."""
    npz_path = Path(npz_path).expanduser()
    if not npz_path.is_absolute():
        npz_path = get_output_paths(output_root, "unused")["npz"].parent / npz_path
...
def parse_args():
    ...
    parser.add_argument("--L", type=int, default=32, help="Default lattice size for relaxation")
    ...
    parser.add_argument("--plot-file", "--load-file", dest="plot_file", type=str, default=None, help="Plot an existing NPZ bundle without recomputing")
    parser.add_argument("--plot-out", type=str, default=None, help="Override PNG output path when using --plot-file")
    parser.add_argument("--show-plot", "--show", dest="show_plot", action="store_true", help="Display the phase-diagram window after saving the PNG")
    ...
    parser.add_argument(
        "--recompute",
        action="store_true",
        help="Accepted for backward compatibility; fresh computation is already the default.",
    )
